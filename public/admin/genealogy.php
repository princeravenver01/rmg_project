...
<div class="content-header"><h2>Genealogy Tree</h2></div>
...
document.addEventListener('DOMContentLoaded', function() {
    const config = {
        initialUserId: <?php echo json_encode(isset($_GET['user_id']) ? (int)$_GET['user_id'] : 20); ?>,
        loggedInUserId: <?php echo json_encode($_SESSION['user_id'] ?? null); ?>,
        apiUrl: 'api_get_genealogy.php',
        ...
        fetch(`${config.apiUrl}?user_id=${userId}`)
            ...
            .then(data => {
                if (data && !data.error) {
                    ...
                    container.innerHTML = `<div class="card"><p style="color: red;">Failed to load tree. The user may not exist in the genealogy.</p></div>`;
                }
            })
    ...
    function buildTreeHtml(node, level) {
        ...
        if (node.id) {
            ...
            if (level === 1 && currentTopNodeUplineId) {
            ...
            nodeContent = `
                <div class="tree-node-wrapper">
                    <a class="tree-node" onclick="${clickAction}">
                        ${(level === 1 && currentTopNodeUplineId) ? '<div class="node-icon" title="Go Up"><i class="fa fa-arrow-up"></i></div>' : '<div class="node-icon"><i class="fa-solid fa-user"></i></div>'}
